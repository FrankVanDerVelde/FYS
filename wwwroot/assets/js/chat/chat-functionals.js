    ...
    // if (FYSCloud.Session.get("loggedin")) {
        ...
        for (let i = 0; i < userChatContact.length; i++) {
            ...
            chatTemplate.addEventListener("click", async function () {
                ...
                //!Double code
                for (let i = 0; i < recipientUser.length; i++) {
                    const message = recipientUser[i];
                    let msgExists = false;
                    ...
                    if (!msgExists) {
                        const msg = `<li class="left-chat" data-id="msg-${message.id}" data-sender="${recieverId}"><p><b>${message.username}</b> :  ${message.message}</p></li>`;
                ...
                for (let i = 0; i < currentUser.length; i++) {
                    ...
                    document.querySelectorAll(".right-chat").forEach(function(element) {
                        if (element.getAttribute('data-reciever') != null) {
                            ...
                            if (element.getAttribute('data-reciever') !=  reciever.id) { 
                                element.style.display = "none";
                            }else{
                                element.style.display = "block";
                            }
                    ...
                    if (!msgExists) {
                        const msg = `<li class="right-chat" data-id="msg-${message.id}" data-reciever="${message.recieverFk}"><p><b>${message.username}</b> :  ${message.message}</p></li>`;
                        document.querySelector(".messages-list").innerHTML += msg;
                    }
                }
            });
